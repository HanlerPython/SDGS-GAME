...
public class PopupManager {
    ...
    private void loadPauseVolumeIcon() {
        try {
            ...
            if (pauseVolumeIconHoverImage == null)
                pauseVolumeIconHoverImage = pauseVolumeIconImage;
        } catch (IOException e) {
    ...
    public void draw(Graphics2D g) {
        ...
        if (currentType == PopupType.UPGRADE) {
            drawUpgradePopup(g);
    ...
    private void drawUpgradePopup(Graphics2D g) {
        ...
        for (int i = 0; i < upgradeOptions.length; i++) {
            ...
            if (upgradeIconBgImage != null) {
                g.drawImage(upgradeIconBgImage, iconBoxX, optionY + (optionHeight - iconBoxSize) / 2, iconBoxSize,
                        iconBoxSize, null);
            } else {
    ...
    private void drawPauseVolumeControl(Graphics2D g) {
        ...
        if (icon != null) {
            ...
            g.drawImage(icon, pauseVolumeIconRect.x, drawY, pauseVolumeIconRect.width, pauseVolumeIconRect.height,
                    null);
        } else {
            g.setColor(Color.WHITE);
    ...
    public void handlePauseMenuClick(int mouseX, int mouseY) {
        if (!isShowing || currentType != PopupType.PAUSE)
            return;
    ...
    public void handleUpgradeClick(int mouseX, int mouseY) {
        if (!isShowing || currentType != PopupType.UPGRADE)
            return;
